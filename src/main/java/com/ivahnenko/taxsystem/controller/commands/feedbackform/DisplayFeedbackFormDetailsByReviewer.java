 ...
 * It executes method for displaying feedback form details for reviewer.
 */
public class DisplayFeedbackFormDetailsByReviewer implements Command {
    ...
    private static final Logger LOGGER = Logger.getLogger(DisplayFeedbackFormDetailsByReviewer.class);

    private FeedbackFormService feedbackFormService;

    public DisplayFeedbackFormDetailsByReviewer() {
        this.feedbackFormService = ServiceFactoryImpl.getInstance().getFeedbackFormService();
    }

    public DisplayFeedbackFormDetailsByReviewer(FeedbackFormService feedbackFormService) {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response)
        ...
        LOGGER.info(String.format("User, id: %d requested DisplayFeedbackFormDetailsByReviewer", user.getId()));

        try {
            int feedbackFormId = Integer.parseInt(request.getParameter(ENTITY_NAME));
            Optional<FeedbackForm> optionalFeedbackForm = feedbackFormService.getByIdAndReviewer(feedbackFormId, user.getId());
