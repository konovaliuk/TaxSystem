 ...
 * It executes method for displaying tax report form details for reviewer.
 */
public class DisplayTaxReportFormDetailsByReviewer implements Command {
    ...
    private static final Logger LOGGER = Logger.getLogger(DisplayTaxReportFormDetailsByReviewer.class);

    private TaxReportFormService taxReportFormService;

    public DisplayTaxReportFormDetailsByReviewer() {
        this.taxReportFormService = ServiceFactoryImpl.getInstance().getTaxReportFormService();
    }

    public DisplayTaxReportFormDetailsByReviewer(TaxReportFormService taxReportFormService) {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response)
        ...
        LOGGER.info(String.format("User, id: %d requested DisplayTaxReportFormDetailsByReviewer", user.getId()));

        try {
            int taxReportFormId = Integer.parseInt(request.getParameter(ENTITY_NAME));
            Optional<TaxReportForm> optionalTaxReportForm = taxReportFormService.getByIdAndReviewer(taxReportFormId, user.getId());

            if (!optionalTaxReportForm.isPresent()) {
                LOGGER.info(String.format("User, id: %d requested taxReportForm details, id: %d by reviewer, id: %1$d = not found", user.getId(), taxReportFormId));
