 ...
 * It executes method for displaying tax report form detailed information.
 */
public class DisplayTaxReportFormDetails implements Command {
    private static final String ENTITY_NAME = "taxReportForm";

    private static final String EXCEPTION_TAX_REPORT_FORM_NOT_FOUND = "exception.exception.documentNotFound";
    ...
    private static final Logger LOGGER = Logger.getLogger(DisplayTaxReportFormDetails.class);

    private TaxReportFormService taxReportFormService;

    public DisplayTaxReportFormDetails() {
        this.taxReportFormService = ServiceFactoryImpl.getInstance().getTaxReportFormService();
    }

    public DisplayTaxReportFormDetails(TaxReportFormService taxReportFormService) {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response)
        ...
        LOGGER.info(String.format("User, id: %d requested DisplayTaxReportFormDetails", user.getId()));

        try {
            int taxReportFormId = Integer.parseInt(request.getParameter(ENTITY_NAME));
            Optional<TaxReportForm> optionalTaxReportForm = taxReportFormService.getById(taxReportFormId);

            if (!optionalTaxReportForm.isPresent()) {
                LOGGER.info(String.format("User, id: %d requested taxReportForm, id: %d = not found", user.getId(), taxReportFormId));
